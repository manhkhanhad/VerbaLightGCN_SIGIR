...
class VerbaLightGCNDataset(Dataset):
    ...
    def __init__(
        ...
        if self.ranking_mode:
            self.test_columns = ['item_1', 'item_2', 'label', 'neighbor_item', 
                      'neighbor_user_1', 'neighbor_user_2', 'candidate', 'pos_index']
        else:
            self.test_columns = ['item_1', 'item_2', 'label', 'neighbor_item',
                      'neighbor_user_1', 'neighbor_user_2']
    ...
    def _generate_sample(self, sample: pd.Series, is_random_item: bool = True) -> Tuple[int, int, int, List[int], List[int], List[int]]:
        ...
        base_return = (item_1, item_2, label, neighbor_item, neighbor_user_1, neighbor_user_2)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        user_id = sample['user_id']
        if self.mode == "train":
            item_1, item_2, label, neighbor_item, neighbor_user_1, neighbor_user_2 = self._generate_sample(sample, is_random_item=True)
            return {
                "user_id": user_id,
                "item_1": item_1,
                "item_2": item_2,
...
class VerbaLightGCNDataModule(pl.LightningDataModule):
    ...
    def generate_graph(self):
        ...
        pair_interaction = pair_interaction[['user_id','train']].explode('train').rename(columns={'train': 'item_id'})
