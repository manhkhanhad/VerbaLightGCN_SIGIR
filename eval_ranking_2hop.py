...
from data.VerbaLightGCN_data_module_2hop import VerbaLightGCNDataModule
import time
import numpy as np
...
def vllm_batch_inferences(model, df, col_name, structure=None, batch_size = 8):
    response = []
...
def generate_item_prompt(row, item_attributes, item_profile, user_profile, agg_item_prompt, test_dataset):
    item_information = item_attributes[row['item_id']]
    profile = json.dumps(item_profile[row['item_id']][0])
    hop1_user = [i for i in row['user_id'] if i in user_profile]
    hop2_item = [int(j.strip("item_id")) for j in test_dataset.get_2hop_neighbors(['user_id_' + str(i) for i in hop1_user]) if j != 'item_id_' + str(row['item_id'])]
    if len(hop2_item) > 10:
        hop2_item = random.choices(hop2_item, k=10, replace=False)

    
    hop1_profile = "\n".join([f"User {i}: " + json.dumps(user_profile[i][0]) for i in hop1_user])
    hop2_profile = "\n".join([f"Item {i}: " + json.dumps(item_profile[i][0]) for i in hop2_item])
    
    return agg_item_prompt.replace("{{{target_item_name}}}", item_information['Title'])\
                            .replace("{{{target_item_characteristic}}}", json.dumps(item_information['Item key characteristics']))\
                            .replace("{{{target_item_profile}}}", profile)\
                            .replace("{{{1hop_user}}}", hop1_profile)\
                            .replace("{{{2hop_item}}}", hop2_profile)

def generate_user_prompt(row, item_attributes, item_profile, user_profile, agg_user_prompt, test_dataset):
    profile = json.dumps(user_profile.get(row['user_id'], ["No information available"])[0])
    hop1_item = [i for i in row['item_id'] if i in item_profile]
    hop2_user = [int(j.strip("user_id")) for j in test_dataset.get_2hop_neighbors(['item_id_' + str(i) for i in hop1_item]) if j != 'user_id_' + str(row['user_id'])]
    if len(hop2_user) > 10:
        hop2_user = random.choices(hop2_user, k=10, replace=False)
    hop1_profile = "\n".join([f"Item {i}: " + json.dumps(item_profile[i][0]) for i in hop1_item])
    hop2_profile = "\n".join([f"User {i}: " + json.dumps(user_profile[i][0]) for i in hop2_user])
    
    return agg_user_prompt.replace("{{{target_user_profile}}}", profile)\
                            .replace("{{{1hop_item}}}", hop1_profile)\
                            .replace("{{{2hop_user}}}", hop2_profile)

def aggregate_profile(config, model, df, item_df, item_attributes, item_profile, user_profile, agg_item_prompt, agg_user_prompt, test_dataset):
    item_df['prompt'] = item_df.apply(lambda x: generate_item_prompt(x, item_attributes, item_profile, user_profile, agg_item_prompt, test_dataset), axis=1)
    
    item_df = vllm_batch_inferences(model, item_df, 'prompt', AggregatedItemProfileResponse)
    agg_item_profile = dict(zip(item_df.item_id, item_df.response))
    df['prompt'] = df.apply(lambda x: generate_user_prompt(x, item_attributes, item_profile, user_profile, agg_user_prompt, test_dataset), axis=1)

    df = vllm_batch_inferences(model, df, 'prompt', AggregatedUserProfileResponse)
...
def generate_ranking_prompt(row, tokenizer, ranking_prompt_template, agg_user_profile, agg_item_profile, item_attributes, ranking_prompt):
    user_profile = json.dumps(agg_user_profile[row['user_id']]) if row['user_id'] in agg_user_profile else ''
    ...
    candidate_text = f"Title: {item_attributes[row['candidate']]['Title']}. Profile: {json.dumps(agg_item_profile[row['candidate']])}"
    prompt = ranking_prompt_template.replace("{{{user_preference}}}", json.dumps(user_profile))\
...
def aggregate_stage(config):
    ...
    agg_item_profile, agg_user_profile = aggregate_profile(config, model, df, item_df, item_attributes, item_profile, user_profile, agg_item_prompt, agg_user_prompt, test_dataset)

    agg_profile = {
        "agg_item_profile": agg_item_profile,
        "agg_user_profile": agg_user_profile
    }
    with open(config["save_agg_profile"], 'w') as f:
        json.dump(agg_profile, f)
...
def ranking(config):
    ...
    # AGGREGATE PROFILE
    print("AGGREGATE PROFILE PATH: ", config["save_agg_profile"])
    with open(config["save_agg_profile"], 'r') as f:
        agg_profile = json.load(f)
    agg_item_profile = {int(k): v for k,v in agg_profile["agg_item_profile"].items()}
    agg_user_profile = {int(k): v for k,v in agg_profile["agg_user_profile"].items()}
...
if __name__ == "__main__":
    ...
    # Toys
    config = {
        "ckpt_path": "ckpt/toys/dev_VerbaLightGCN_v2_multiView_k3_run2/dev_VerbaLightGCN_v2_multiView_k3_run2-epoch=04-val_f1=0.00.ckpt",
