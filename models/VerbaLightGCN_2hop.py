...
class AggregatedUserProfileResponse(BaseModel):
    UserPreferences: str
    HighOrderCFView: str

class AggregatedItemProfileResponse(BaseModel):
    SemanticView: str
...
class UpdateDescriptionResponse(BaseModel):
    Explanation: str
    UpdatedUserProfile: AggregatedUserProfileResponse
    UpdatedItem1Profile: AggregatedItemProfileResponse
    UpdatedItem2Profile: AggregatedItemProfileResponse
...
class VerbaLightGCN(pl.LightningModule):
        ...
        **kwargs: Any,
        ...
        if self.hparams.item_profile_path is not None:
            ...
            for item, item_info in self.item_attributes.items():
                self.item_memory[item] = [{"SemanticView": "Item attributes: " + ", ".join(json.loads(item_info)['Item key characteristics'])}]
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        batch = self._aggregate_node_profile(batch)
    ...
    def optimizer_step(self, outputs: Dict[str, Any], batch: Dict[str, Any]) -> None:
        ...
        for i in range(len(batch['predict_prompt'])):
            ...
            try:
                ...
                aggregated_user = batch['aggregated_user_profile'][i]
                aggregated_item1 = batch['aggregated_item1_profile'][i]
                aggregated_item2 = batch['aggregated_item2_profile'][i]
                ...
                optimizer_prompt = optimizer_prompt_template.replace("{{{user_profile}}}", json.dumps(self.user_memory[user_id][-1]) if user_id in self.user_memory else 'No information')\
                                        ...
                                        .replace("{{{item_1_profile}}}", json.dumps(self.item_memory[item_1][-1]) if item_1 in self.item_memory else 'No information')\
                                        ...
                                        .replace("{{{item_2_profile}}}", json.dumps(self.item_memory[item_2][-1]) if item_2 in self.item_memory else 'No information')\
                                        ...
                                        .replace("{{{aggregated_user}}}", json.dumps(batch['aggregated_user_profile'][i]) if batch['aggregated_user_profile'][i] is not None else 'No information')\
                                        .replace("{{{aggregated_item1}}}", json.dumps(batch['aggregated_item1_profile'][i]) if batch['aggregated_item1_profile'][i] is not None else 'No information')\
                                        .replace("{{{aggregated_item2}}}", json.dumps(batch['aggregated_item2_profile'][i]) if batch['aggregated_item2_profile'][i] is not None else 'No information')
        ...
        for i, update_description in enumerate(refinement_outputs):
            ...
            if label != outputs['llm_predictions'][i] or self.trainer.current_epoch == 0: # Only update profile if it false
                logger.info(f"update user {user_id} profile " + (json.dumps(self.user_memory[user_id][-1]) if user_id in self.user_memory else 'No information') + " -> " + json.dumps(update_description['UpdatedUserProfile']))
                ...
                if label == 1: # Update the positive item profile
                    logger.info(f"update item {item_1} profile " + (json.dumps(self.item_memory[item_1][-1]) if item_1 in self.item_memory else 'No information') + " -> " + json.dumps(update_description['UpdatedItem1Profile']))
                    ...
                    logger.info(f"update item {item_2} profile " + (json.dumps(self.item_memory[item_2][-1]) if item_2 in self.item_memory else 'No information') + " -> " + json.dumps(update_description['UpdatedItem2Profile']))
            ...
            if self.log_training == True:
                self.train_content['user_profile_update'].append(json.dumps(update_user_profile) if update_user_profile != "None" else 'No information')
                self.train_content['item_1_profile_update'].append(json.dumps(update_item_1_profile) if update_item_1_profile != "None" else 'No information')
                self.train_content['item_2_profile_update'].append(json.dumps(update_item_2_profile) if update_item_2_profile != "None" else 'No information')
                self.train_content['llm_predictions'].append(outputs['llm_predictions'][i])
                self.train_content['llm_explanations'].append(outputs['llm_explanations'][i])
                self.train_content['optimizer_prompts'].append(optimizer_prompt_list[i])
    ...
    def _preprocess_batch(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        neighbor_item_list = []
        neighbor_user_1_list = []
        neighbor_user_2_list = []
        for i in range(len(batch['user_id'])):
            ...
            neighbor_user_1 = [i for i in batch["neighbor_user_1"][i].cpu().tolist() if i != 0 and i in self.user_memory]
            neighbor_user_2 = [i for i in batch["neighbor_user_2"][i].cpu().tolist() if i != 0 and i in self.user_memory]
            neighbor_item_list.append(neighbor_item)
            neighbor_user_1_list.append(neighbor_user_1)
            neighbor_user_2_list.append(neighbor_user_2)
        batch['neighbor_item'] = neighbor_item_list
        batch['neighbor_user_1'] = neighbor_user_1_list
        batch['neighbor_user_2'] = neighbor_user_2_list
        return batch

    def _aggregate_user_node_profile(self, batch) -> str:
        prompt_list = []
        for i in range(len(batch['user_id'])):
            user_id = batch['user_id'][i].item()
            neighbor_item = batch['neighbor_item'][i].cpu().numpy().tolist()
            user_2hop_node = batch['user_2hop_node'][i].cpu().numpy().tolist()
            item_1hop_profile = ["- Item Title: " + json.loads(self.item_attributes[iid])['Title'] + " (" + self.item_memory[iid][-1]['SemanticView'] + ")" for iid in neighbor_item if iid in self.item_memory]
            user_2hop_profile = ["- " + self.user_memory[uid][-1]['UserPreferences'] for uid in user_2hop_node if uid in self.user_memory]
            prompt_list.append(
                self.aggregated_user_prompt.replace("{{{target_user_profile}}}", json.dumps(self.user_memory[user_id][-1]) if user_id in self.user_memory else 'No information')\
                .replace("{{{1hop_item}}}", "\n".join(item_1hop_profile))\
                .replace("{{{2hop_user}}}", "\n".join(user_2hop_profile)))
        
        guided_decoding_params = GuidedDecodingParams(json=AggregatedUserProfileResponse.model_json_schema())
        conversation = []
        for p in prompt_list: # Only consider non-empty prompts
            conversation.append([{"role": "system", "content": "You are a helpful assistant."},
                                {"role": "user", "content": p[:32768]}])
        llm_outputs = self._call_vllm_inference(conversation, max_tokens=1024, guided_decoding_params=guided_decoding_params)
        result_agg_user_profile = []
        for i, input_prompt in enumerate(prompt_list):
            try:
                result_agg_user_profile.append(json.loads(llm_outputs[i].outputs[0].text))
            except json.JSONDecodeError:
                result_agg_user_profile.append({
                    "UserPreferences": "No information",
                    "HighOrderCFView": "No information"
                })
        return result_agg_user_profile
    
    def _aggregate_item_node_profile(self, batch) -> str:
        prompt_list_item_1 = []
        prompt_list_item_2 = []
        for i in range(len(batch['item_1'])):
            item_1 = batch['item_1'][i].item()
            item_1_2hop_node = batch['item_1_2hop_node'][i].cpu().numpy().tolist()
            neighbor_user_1 = [i for i in batch["neighbor_user_1"][i].cpu().tolist() if i != 0 and i in self.user_memory]
            item_1_1hop_node_profile = ["- " + self.user_memory[uid][-1]['UserPreferences'] for uid in neighbor_user_1 if uid in self.user_memory]
            item_1_2hope_node_profile = ["- " + self.item_memory[iid][-1]['SemanticView'] for iid in item_1_2hop_node if iid in self.item_memory]

            prompt_list_item_1.append(
                self.aggregated_item_prompt.replace("{{{target_item_name}}}", json.loads(self.item_attributes[item_1])['Title'])\
                .replace("{{{target_item_characteristic}}}", "; ".join(json.loads(self.item_attributes[item_1])['Item key characteristics']))\
                .replace("{{{target_item_profile}}}", json.dumps(self.item_memory[item_1][-1]) if item_1 in self.item_memory else 'No information')\
                .replace("{{{1hop_user}}}", "\n".join(item_1_1hop_node_profile))\
                .replace("{{{2hop_item}}}", "\n".join(item_1_2hope_node_profile)))


            item_2 = batch['item_2'][i].item()
            item_2_2hop_node = batch['item_2_2hop_node'][i].cpu().numpy().tolist()
            neighbor_user_2 = [i for i in batch["neighbor_user_2"][i].cpu().tolist() if i != 0 and i in self.user_memory]
            item_2_1hop_node_profile = ["- " + self.user_memory[uid][-1]['UserPreferences'] for uid in neighbor_user_2 if uid in self.user_memory]
            item_2_2hope_node_profile = ["- " + self.item_memory[iid][-1]['SemanticView'] for iid in item_2_2hop_node if iid in self.item_memory]

            prompt_list_item_2.append(
                self.aggregated_item_prompt.replace("{{{target_item_name}}}", json.loads(self.item_attributes[item_2])['Title'])\
                .replace("{{{target_item_characteristic}}}", "; ".join(json.loads(self.item_attributes[item_2])['Item key characteristics']))\
                .replace("{{{target_item_profile}}}", json.dumps(self.item_memory[item_2][-1]) if item_2 in self.item_memory else 'No information')\
                .replace("{{{1hop_user}}}", "\n".join(item_2_1hop_node_profile))\
                .replace("{{{2hop_item}}}", "\n".join(item_2_2hope_node_profile)))

        prompt_list = prompt_list_item_1 + prompt_list_item_2
        guided_decoding_params = GuidedDecodingParams(json=AggregatedItemProfileResponse.model_json_schema())
        conversation = []
        for p in prompt_list: # Only consider non-empty prompts
            conversation.append([{"role": "system", "content": "You are a helpful assistant."},
                                {"role": "user", "content": p[:32768]}])
        llm_outputs = self._call_vllm_inference(conversation, max_tokens=1024, guided_decoding_params=guided_decoding_params)
        result_agg_item_profile = []
        BS = len(prompt_list) // 2
        for i, input_prompt in enumerate(prompt_list):
            try:
                result_agg_item_profile.append(json.loads(llm_outputs[i].outputs[0].text))
            except json.JSONDecodeError:
                result_agg_item_profile.append({
                    "SemanticView": "No information",
                    "CollaborativeView": "No information",
                    "HighOrderCFView": "No information"
                })
        return result_agg_item_profile[:BS], result_agg_item_profile[BS:]

    def _aggregate_node_profile(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        agg_user_profile_list = self._aggregate_user_node_profile(batch)
        agg_item_1_profile_list, agg_item_2_profile_list = self._aggregate_item_node_profile(batch)
        batch['aggregated_user_profile'] = agg_user_profile_list
        batch['aggregated_item1_profile'] = agg_item_1_profile_list
        batch['aggregated_item2_profile'] = agg_item_2_profile_list
        return batch
    ...
    def _construct_prompts(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for i in range(len(batch['user_id'])):
            ...
            predict_prompt = self.prompt_template.replace("{{{user_profile}}}", json.dumps(self.user_memory[user_id][-1]) if user_id in self.user_memory else 'No information')\
                                    .replace("{{{user_hist}}", self.construct_neighbor_item_feature(neighbor_item))\
                                    .replace("{{{item_1_information}}}", self.item_attributes[item_1])\
                                    .replace("{{{item_1_profile}}}", json.dumps(self.item_memory[item_1][-1]) if item_1 in self.item_memory else 'No information')\
                                    .replace("{{{user_like_item_1}}}", self.construct_neighbor_user_feature(neighbor_user_1))\
                                    ...
                                    .replace("{{{item_1_description}}}", self.item_description['description'][str(item_1)])\
                                    .replace("{{{item_2_description}}}", self.item_description['description'][str(item_2)])\
                                    .replace("{{{user_agg_profile}}}", json.dumps(batch['aggregated_user_profile'][i]) if batch['aggregated_user_profile'][i] is not None else 'No information')\
                                    .replace("{{{aggregated_item1}}}", json.dumps(batch['aggregated_item1_profile'][i]) if batch['aggregated_item1_profile'][i] is not None else 'No information')\
                                    .replace("{{{aggregated_item2}}}", json.dumps(batch['aggregated_item2_profile'][i]) if batch['aggregated_item2_profile'][i] is not None else 'No information')
            ...
            if self.log_training == True and self.trainer.training:
                ...
                self.train_content['user_profile'].append(json.dumps(self.user_memory[user_id][-1]) if user_id in self.user_memory else 'No information')
                ...
                self.train_content['item_1_profile'].append(json.dumps(self.item_memory[item_1][-1]) if item_1 in self.item_memory else 'No information')
                self.train_content['item_2_profile'].append(json.dumps(self.item_memory[item_2][-1]) if item_2 in self.item_memory else 'No information')
    ...
    def construct_neighbor_item_feature(self, item_list):
        hist_text = []
        for i in item_list:
            # Use .get() for safety, though items should be initialized in item_memory
    ...
    def on_validation_epoch_start(self):
        if self.trainer.global_step > 0: # Only summarize profile when training and before validation epoch
    ...
    def _load_prompts(self):
        ...
        with open(os.path.join(self.hparams.prompt_path, 'aggreate_user_profile.txt'), 'r') as f:
            self.aggregated_user_prompt = f.read()
        with open(os.path.join(self.hparams.prompt_path, 'aggreate_item_profile.txt'), 'r') as f:
            self.aggregated_item_prompt = f.read()
    ...
    def summarize_memory(self, summary_user_mem=False, summary_item_mem=False, max_profile_length=2):
        ...
        def build_summary_user_prompt(profile_dict, entity_type):
            ...
            for eid in ids_to_summarize:
                memory = "- " + "\n- ".join([json.dumps(profile) for profile in profile_dict[eid]])
                if eid in self.user_profile_ori:
                    memory = f"- {json.dumps(self.user_profile_ori[eid])}\n" + memory

                user_hist = []
                for i in self.user_hist_cache[eid][-30:]: # Only use the last 30 items
                    his_item_info = {
                        "title": self.item_description['title'][str(i)],
                        "description": json.dumps(self.item_memory[i][-1]['SemanticView']) if i in self.item_memory else ', '.join(json.loads(self.item_attributes[i])['Item key characteristics'])
                    }
        ...
        if len(item_prompts) > 0 and summary_item_mem:
            ...
            item_summaries = self.batch_summarize_profiles(item_prompts, max_tokens=512, schema=AggregatedItemProfileResponse)
            for eid, summary in zip(item_ids, item_summaries):
                if summary != "None":
                    logger.info(f"Item {eid} CF profile summarized to: {summary}")
                    self.item_memory[eid] = [summary]
        else:
        ...
        if len(user_prompts) > 0 and summary_user_mem:
            ...
            user_summaries = self.batch_summarize_profiles(user_prompts, max_tokens=512, schema=AggregatedUserProfileResponse)
            
            for eid, summary in zip(user_ids, user_summaries):
                logger.info(f"User {eid} CF profile summarized to: {summary}")
                if summary != "None":
                    self.user_memory[eid] = [summary]
        else:
            logger.info("No user collaborative filtering profiles to summarize.")

    def batch_summarize_profiles(self, prompts: list, max_tokens: int = 512, chunk_size: int = 12, schema: BaseModel = AggregatedUserProfileResponse) -> list:
        """
